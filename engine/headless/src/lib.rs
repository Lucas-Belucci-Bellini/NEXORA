...
pub struct SliceReport {
    ...
    pub frames: u64,
    /// Fixed steps those frames ran. One step is one streaming tick.
    pub frame_steps: u64,
    /// Steps the frame loop's catch-up cap discarded.
    ///
    /// Zero here, and that is the point: the walk is driven by a scripted
    /// delta of exactly one step, so it can never fall behind its own
    /// schedule. A non-zero value would mean the slice stopped being the
    /// deterministic proof it is meant to be.
    pub frame_steps_dropped: u64,
    /// Frames whose input stage was sampled. One per frame: the input system
    /// runs at the top of every frame whether or not anything was pressed.
    pub input_frames: u64,
    /// Frames whose snapshot carried an intent - a press, a release, or a
    /// stick off centre. Sixteen here: the scripted player taps a key once per
    /// leg of the walk and lets go on the next frame.
    pub input_intents: u64,
...
pub fn run_slice(config: &SliceConfig) -> Result<SliceReport> {
    ...
    Ok(SliceReport {
        ...
        frame_steps_dropped: streaming.steps_dropped,
        input_frames: streaming.input_frames,
        input_intents: streaming.input_intents,
...
pub fn format_report(report: &SliceReport) -> String {
    ...
    out.push_str(&format!(
        "frames             {} ({} fixed steps at {} Hz, {} dropped)\n",
        report.frames, report.frame_steps, DEFAULT_TICKS_PER_SECOND, report.frame_steps_dropped
    ));
    out.push_str(&format!(
        "input              {} frames sampled, {} carried an intent\n",
        report.input_frames, report.input_intents
    ));
    out.push_str(&format!(
...
/// The scancode a host would report for the forward key. Opaque to the engine
/// on purpose - `INPUT SYSTEM.md` and `CORE.md` §24 both insist the game does
/// not depend on keys, so the engine never learns that 17 means `W`.
const WALK_FORWARD: ButtonCode = ButtonCode(17);

/// The scancode for the back key.
const WALK_BACK: ButtonCode = ButtonCode(31);

/// The walk the slice takes, expressed as something a player did.
///
/// This used to be a `Vec<i64>` of stops written down in `stream_a_walk`. It is
/// now the output of the engine's input system (`INPUT SYSTEM.md`, ENGINE-8):
/// two keys bound to one movement axis, one of them inverted, tapped once per
/// leg. The route that comes out is checked against the closed form it replaced,
/// so this is the same walk and not a similar one - and if a leg ever runs too
/// few frames for the player to let go of the key, the missing press shows up
/// as a route that does not match rather than as a quietly shorter walk.
///
/// What it demonstrates is the rule the input system exists to keep:
/// **input produces intent, and something else decides what the intent means**.
/// The system here says "the player is asking to move forward". Nothing inside
/// it knows there is a world, a chunk, or an observer.
struct ScriptedWalk {
    input: InputSystem,
    axis: Identifier,
    keyboard: DeviceId,
    stride: i64,
    stop: i64,
    presses: Vec<ButtonCode>,
    next_press: usize,
    held: Option<ButtonCode>,
    attached: bool,
    intents: u64,
}

impl ScriptedWalk {
    /// Bind the keys and script the taps: `strides` legs out, then the same back.
    fn new(stride: i64, strides: usize) -> Result<Self> {
        let axis = Identifier::nexora("action/walk")?;
        let context = Identifier::nexora("context/gameplay")?;
        let mut input = InputSystem::new();
        input.register_action(ActionDefinition::new(axis.clone(), ActionKind::Axis))?;
        ...
        let mut presses = vec![WALK_FORWARD; strides];
        presses.extend(std::iter::repeat_n(WALK_BACK, strides));
        Ok(Self {
            input,
            axis,
    ...
    /// Sample one frame of input and return the stop the player is asking for.
    ///
    /// `opens_a_leg` is the only thing the caller has to say: on that frame the
    /// player taps the next key, and on the frame after it they let go.
    fn frame(&mut self, opens_a_leg: bool) -> i64 {
        ...
        if let Some(code) = self.held.take() {
            signals.push(Signal::button(self.keyboard, code, false));
        }
        if opens_a_leg {
            if let Some(code) = self.presses.get(self.next_press).copied() {
                self.next_press += 1;
                signals.push(Signal::button(self.keyboard, code, true));
                self.held = Some(code);
            }
        }

        let snapshot = self.input.sample(&signals);
        if !snapshot.is_empty() {
            self.intents += 1;
        }
        if snapshot.just_pressed(&self.axis) {
            let direction = if snapshot.axis(&self.axis) > 0.0 {
                1
            } else {
                -1
            };
            self.stop += direction * self.stride;
        }
        self.stop
    }

    /// The route the closed form used to produce, for the slice to check against.
    fn expected_route(stride: i64, strides: usize) -> Vec<i64> {
        let legs = i64::try_from(strides).unwrap_or(0);
        let mut route: Vec<i64> = (0..=legs).map(|leg| leg * stride).collect();
        route.extend((0..legs).rev().map(|leg| leg * stride));
        route
    }
}
...
fn stream_a_walk(
    ...
    // The walk runs on the engine's frame loop (`CORE.md` §16) rather than on a
    // bare counter. The step is the world clock's own tick rate, so one fixed
    // step is one streaming tick and simulated time is a unit the settle bound
    // can be expressed in: the loop below used to stop after 256 iterations, a
    // number with no unit; the same bound at 20 Hz is 12.8 s of simulated time.
    //
    // The delta handed in is exactly one step, which keeps the slice
    // deterministic - it is a proof, not a measurement, and a slice whose step
    // count depended on how fast the machine ran it could not be compared
    // across runs. The real distribution is measured in the benchmark suite
    // instead. What the frame loop earns here is the attribution: the time the
    // streaming tick spends is charged to the `World` stage and nowhere else.
    let step = Duration::from_secs(1) / DEFAULT_TICKS_PER_SECOND;
    const SETTLE_LIMIT: Duration = Duration::from_millis(12_800);
    let mut engine = FrameLoop::new(
        FrameSchedule::new(step, 4)?,
        FrameBudget::doubling_from(step),
    );
    let mut steps = 0_u64;
    let mut steps_dropped = 0_u64;

    // Out four strides, then back to where it started. The stops are no longer
    // written down: they are what the input system says the player asked for.
    // See `ScriptedWalk`.
    const STRIDES: usize = 4;
    let stride = radius + 1;
    let mut walk = ScriptedWalk::new(stride, STRIDES)?;
    let mut route_taken: Vec<i64> = Vec::with_capacity(STRIDES * 2 + 1);

    for leg in 0..=STRIDES * 2 {
        ...
        while !settled && simulated < SETTLE_LIMIT {
            let frame_began = Instant::now();
            let mut frame = engine.begin(step);
            let plan = frame.plan();
            steps_dropped += plan.discarded;

            // Input runs at the top of the frame, which is where `CORE.md` §16
            // puts it and why `FrameStage::SEQUENCE` begins there. The first leg
            // does not move - the observer starts where it starts - so it taps
            // nothing and the walk opens at the origin.
            let opens_a_leg = opening && leg > 0;
            let input_began = Instant::now();
            let stop = walk.frame(opens_a_leg);
            frame.record(FrameStage::Input, input_began.elapsed())?;

            let world_began = Instant::now();
            if opening {
                // Acting on the intent is the slice's own work, not the input
                // system's: the intent says "forward", and this is the line
                // that decides forward means an interest source three chunks
                // further out.
                system.set_interest(
                    InterestSource::new(InterestId(1), ChunkCoord::new(stop, 0)).with_radii(radii),
                )?;
                route_taken.push(stop);
                opening = false;
            }
            ...
            frame.record(FrameStage::World, world_began.elapsed())?;
            let frame_report = frame.finish(frame_began.elapsed());

            // The stages cannot together outlast the frame that contains them.
            // Checked rather than assumed: this is the one place the slice runs
            // the attribution against real work, and an accounting bug that
            // only shows up under a real workload would otherwise show up as a
            // plausible number in a report nobody could refute.
            if !frame_report.overattributed().is_zero() {
                return Err(Error::new(
                    Domain::Core,
                    "slice/frame",
                    "frame stages were charged more time than the frame lasted",
                )
                .with_recovery(Recovery::Reject)
                .with_context("frame", frame_report.number().to_string()));
            }
        }
    ...
    // The walk the player asked for has to be the walk this stage has always
    // taken, or every number below it means something different. A leg that ran
    // too few frames for the player to release the key would produce a missed
    // press and a shorter route - which is a failure here rather than a quietly
    // different proof.
    let expected = ScriptedWalk::expected_route(stride, STRIDES);
    if route_taken != expected {
        return Err(Error::new(
            Domain::Input,
            "slice/input",
            "the scripted player did not walk the route the slice is specified to walk",
        )
        .with_recovery(Recovery::Manual)
        .with_context("walked", format!("{route_taken:?}"))
    ...
    Ok(StreamingOutcome {
        ticks,
        input_frames: walk.input.frames(),
        input_intents: walk.intents,
