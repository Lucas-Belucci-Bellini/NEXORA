...
contract_version!(
    /// Version of the logical schema of a material document.
    ///
    /// `NEXORA CONTENT PIPELINE SPECIFICATION.md` requires every persistent
    /// resource to carry a stable id and a validated shape; a document whose
    /// layout changes without changing this number is a document that will be
    /// read wrong by a build that predates the change.
    MaterialSchemaVersion,
    "material-schema"
);

contract_version!(
    /// Version of one content generator's algorithm.
    ///
    /// Deliberately distinct from [`GeneratorVersion`], which versions **world
    /// generation**. `NEXORA NAMING AND TERMINOLOGY.md` forbids one name
    /// covering two concepts, and these two move independently: changing how
    /// stone is drawn must not imply that every saved world was generated by a
    /// different algorithm.
...
contract_version!(
    /// Version of one content pipeline's transformation.
    ///
    /// Recorded in provenance so that "how was this asset produced?" names the
    /// pipeline as well as the generator: the same generator run through a
    /// changed pipeline does not produce the same bytes.
    ContentPipelineVersion,
    "content-pipeline"
);
